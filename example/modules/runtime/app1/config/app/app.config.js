 ...
 * 也可以直接赋值
 */
let AppConfiguration = class AppConfiguration extends src_1.PYIAutoAppConfiguration {
    /**
     * 赋值注入
     */
    ...
    constructor(config, props) {
        super(config, props);
        this.development.output = path_1.join(config.entry, '../../runtime/app1');
        this.production.output = path_1.join(config.entry, '../../runtime/app1');
        this.development.pyi = this.production.pyi = this.default.pyi = {
            routePrefix: '/app1'
        };
        this.development.server.port = this.development.server.port = this.development.server.port = 4003;
        /**
         * 赋值注入
         */
...
AppConfiguration = __decorate([
    src_1.Configuration,
    __metadata("design:paramtypes", [src_1.AppConfigOption, Object])
], AppConfiguration);
